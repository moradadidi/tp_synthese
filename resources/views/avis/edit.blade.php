...
<div class="container mx-auto px-4 py-6">
    <div class="max-w-xl mx-auto bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6">
        <h2 class="text-2xl font-bold mb-4 text-gray-800 dark:text-white">Modifier un avis</h2>

        <form method="POST" action="{{ route('avis.update', $avis->id) }}" class="space-y-4">
            @csrf
            @method('PUT')
                ...
                >
                    ...
                    @foreach($etudiants as $etudiant)
                        <option
                            ...
                            {{ old('ide', $avis->ide) == $etudiant->codeE ? 'selected' : '' }}
                        >
            ...
            <div>
                ...
                >
                    ...
                    @foreach($formations as $formation)
                        <option
                            value="{{ $formation->idf }}"
                            {{ old('idf', $avis->idf) == $formation->idf ? 'selected' : '' }}
                        >
                            {{ $formation->titre }}
                        </option>
                    @endforeach
